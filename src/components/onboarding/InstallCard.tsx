...
import { isIOS, isIOSSafari, isStandalone } from '@/lib/install'
 ...
 *
 * **סדר הענפים הוא העיקר, לא העיצוב.** "אייפון שאינו ספארי" נבדק לפני הוראות
 * ההתקנה: קישור שנפתח מוואטסאפ רץ בדפדפן מוטמע שאין בו "הוסף למסך הבית"
 * בכלל, וגם בכרום ובפיירפוקס באייפון המסלול האמין היחיד הוא לעבור לספארי.
 * כרטיס שמסביר איפה הפריט יושב היה שולח את מי שקיבל את הקישור לחפש כפתור
 * שלא קיים במסך שלפניו — וזה מסלול ברירת המחדל של ההפצה כאן, לא מקרה קצה.
 */
  ...
  if (isIOS() && !isIOSSafari()) {
    return (
      <Frame icon={<Compass size={18} />} title="קודם פותחים בספארי">
        <p>
          המסך הזה לא פתוח עכשיו בספארי, ובאייפון רק משם אפשר להתקין למסך הבית.
        </p>
        <ol className="space-y-1.5">
          <Step n={1}>
            לחץ על כפתור השיתוף{' '}
            <Share size={14} className="inline align-[-2px] text-flame-400" /> או על תפריט
            שלוש הנקודות
          </Step>
          <Step n={2}>
