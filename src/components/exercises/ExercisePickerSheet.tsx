...
export interface ExercisePickerSheetProps {
  ...
  title: string
  /**
   * מזהי תרגיל שכבר נמצאים ביעד. השורה מסומנת ✓ ונשארת לחיצה — אותו תרגיל
   * פעמיים באימון הוא בקשה לגיטימית (סופרסט, סבב שני), ולא תקלה למנוע.
   */
    ...
    <>
      ...
      */}
      ...
      {/*
        ...
        זה לא סגנון: שניהם נועלים את גלילת ה-body ושומרים את הערך הקודם, וכשהם
        מקוננים React מריץ את ניקויי האפקטים מההורה לילד בהסרת עץ. הגיליון
        משחרר את הנעילה, והנגן כותב אחריו בחזרה את ה-'hidden' שהוא תפס ממנו —
        והדף נשאר תקוע בלי גלילה. כאחים הסדר הפוך והבעיה לא נוצרת.
      */}
      {gallery ? (
        <VideoPlayer
