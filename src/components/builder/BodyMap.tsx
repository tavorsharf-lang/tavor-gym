 ...
 *
 * הרשימה עונה על "על מה לא עבדתי" במילים, ומי שקורא אותה סורק שמונה שורות
 ...
 *
 * ‏1. האזורים מצוירים בגסות ובגדול, יותר מהאנטומיה האמיתית. אצבע על מסך טלפון
 *    צריכה לפגוע במטרה, ושריר בגודלו האמיתי הוא יעד של כמה פיקסלים.
 ...
 *    מכיר `legs` אחד, ואם המפה הייתה מתייחסת לארבע ראשי, ירך אחורית ותאומים
 *    כמצבים שונים היא הייתה ממציאה נתון שאינו קיים.
 *
 ...
 *    (כתום = אומן), ואז אותם שמונה שרירים בדיוק נראו "כבויים" במפה ו"דולקים"
...
/** מאחור: גב ויד אחורית — והרגליים והכתפיים שוב, כאותה קבוצה */
function BackFigure({ rows, onSelect }: FigureProps): JSX.Element {
  return (
    <g>
      ...
      {/* עכוז, המסטרינג ותאומים — אזור אחד, כי שוק אינו קבוצה בפני עצמה */}
      <Region group="legs" rows={rows} onSelect={onSelect}>
