...
export interface ActiveWorkout {
   ...
   * האם התור נבחר מראש.
   *
   * ‏`routineId === null` נשאר מה שהופך אימון ל"אימון חופשי" בהיסטוריה
   * ובסיכום — אבל הוא לא מספיק למסך האימון. גם הבונה וגם "בנה לי אימון"
   * פותחים אימון בלי תוכנית **ועם תור שלם שנבחר מראש**, ובלי ההבחנה הזו
   * המסך הציג את חמשת התרגילים שנבחרו בסל כאילו אין אף אחד מהם.
   *
   * אופציונלי כי אימון פתוח שנשמר לפני השדה מגיע בלעדיו — `hydrate` משלים
   * אותו מאורך התור, וזו הערכה נכונה לכל אימון שכבר רץ.
   */
  planned?: boolean
}
