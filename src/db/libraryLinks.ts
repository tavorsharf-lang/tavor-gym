...
export const LIBRARY_LINKS: Readonly<Record<string, string>> = {
  ...
  // שני אלה נפתחו רק בפיצול לחיצת החזה (מיגרציה 10). כל עוד רשומה אחת כיסתה
  // גם שטוח וגם שיפוע לא היה לה למה להתקשר — המאגר מפריד ביניהן, והרשומה
  // השטוחה שם מלמדת להוריד את המוט לחזה האמצעי, הפוך ממה שנכון בשיפוע.
  'db-bench-press': 'lib-barbell_bench_press',
  'incline-barbell-bench-press': 'lib-incline_barbell_bench_press',
  // `lib-calf_raise` נפתח באודיט הרגליים. ההערה שהייתה כאן אמרה בדיוק את זה —
  // "האזכור היחיד הוא לחיצת שוק על מזחלת ה-leg press" — והקליפ הזה הוא בדיוק
  // אותו קליפ, שישב עד עכשיו תחת לחיצת רגליים.
...
export const UNLINKED_NOTES: Readonly<Record<string, string>> = {
  /*
    ההערה כאן אמרה עד כה "אין כפיפת פטיש במאגר; כל 27 סרטוני הכפיפה שם באחיזת
    סופינציה". החלק הראשון כבר לא נכון — `lib-incline_hammer_curl` נפתח מקליפ
    שהמקור סיווג לפי הכותרת ("Incline Dumbbell Curl") בזמן שהביצוע ניטרלי.
    מה שמונע קישור הוא אחר, והוא עדיין עומד: זו כפיפה בשיפוע וזו בישיבה.
  */
  'hammer-curl':
    'הפטיש היחיד במאגר הוא בשיפוע (lib-incline_hammer_curl), וזה תרגיל אחר — הזרוע שם מאחורי קו הגוף',
  // האודיט הראה שרשומת האמות מכסה גם כפיפה וגם פשיטה, ולכן ההערה הישנה
  // ("רק פשיטה — התנועה ההפוכה") לא הייתה נכונה. מה שמונע קישור הוא אחר:
  // שני התרגילים האלה הם ציוד שונה, ושניהם היו נקשרים לאותה רשומה יחידה —
  // בדיוק הכשל ש-`seated-row-heavy`/`seated-row-light` מזהירים ממנו.
  'forearm-straight-bar':
    'רשומת האמות היחידה במאגר מכסה גם כפיפה וגם פשיטה, ומערבבת דאמבל, כבל ומוט — קישור שלה גם לכאן וגם ל-forearm-dumbbell היה מוביל שני תרגילים שונים לאותו מקום',
  'forearm-dumbbell':
    'אותה סיבה כמו forearm-straight-bar — רשומת אמות אחת במאגר מול שני תרגילים בציוד שונה בקטלוג',
  'seated-row-heavy':
