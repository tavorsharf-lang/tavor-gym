...
const RAW: ExerciseSeed[] = [
  ...
  {
    /**
     * שטוח ושיפוע חיובי היו רשומה אחת עד מיגרציה 10, כי כך נראתה תיקיית המקור:
     * ארבעה סרטונים תחת "לחיצת חזה חופשי", שלושה שטוח ואחד בשיפוע. זה עבד כשם
     * ונשבר בכל השאר — הדגשים סתרו את עצמם (המוט יורד לחזה האמצעי בשטוח ולעליון
     * בשיפוע), והמאגר הלימודי מחזיק שתי רשומות נפרדות ולכן אף אחת מהן לא יכלה
     * להתקשר. עכשיו זו הרשומה השטוחה בלבד, ו-`incline-barbell-bench-press` היא
     * התאומה שלה.
     *
     * המזהה נשאר `db-bench-press` למרות ש-"db" הוא שריד מהתקופה שבה הרשומה
     * נקראה בטעות כלחיצת דאמבלים: המזהה הוא המפתח של ההיסטוריה, השיאים,
     * הסרטונים והתוכניות, ושינוי שלו היה מנתק את כולם. שם מטעה עדיף על נתונים
     * שאבדו.
     */
    id: 'db-bench-press',
  ...
  },
  {
    ...
    name: 'לחיצת חזה במוט בשיפוע חיובי',
    ...
    targetReps: reps(8, 12),
    cues: [
      'משענת ב-30–40 מעלות. תלול מזה והלחיצה עוברת לכתף הקדמית',
      'שכמות נעוצות במשענת לאורך כל הסט',
      'להוריד את המוט לחזה העליון, מתחת לעצם הבריח',
      'מרפקים ב-45 מעלות מהגוף — האמה אנכית בתחתית',
    ],
    usesPlates: true,
    barWeightKg: null,
    seedWeightKg: 22.5,
  ...
  {
    /**
     * הקבוצה הראשית הייתה `chest` עד מיגרציה 10, שריד מהשם הישן "מקבילים".
     * הסרטון מוכתר "Plate Loaded Tricep Dips" וכל הנחיה בו מרחיקה עומס מהחזה
     * — לא לשקוע עמוק ("you'll get your pecs too much"), גוף זקוף, מרפקים
     * צמודים. זה תרגיל טריצפס, והחזה הוא העבודה המשנית.
     */
    id: 'dips',
    name: 'מקבילים במכונה',
    nameEn: 'Plate-Loaded Chest Dips',
    /*
      חזה ולא טריצפס, וזו חזרה מהחלטה של מיגרציה 10.

      שם ההכרעה נשענה על כותרת הסרטון ("Plate Loaded TRICEP Dips") ועל הדגשים
      שבו. מאז נכנס למאגר כרטיס השרירים של המכונה הזו, והוא אומר משהו אחר:
      **חזה תחתון 48%** מול טרייספס 27%, כתף קדמית 15% ומייצבים 10%. הכרטיס
      הוא המקור שמסך הכיסוי ומיון האחוז קוראים ממנו, ולכן קבוצה שסותרת אותו
      פירושה שהאפליקציה סופרת את התרגיל תחת שריר שהוא לא הראשי שלו.

      ‏`lib-dips` — מקבילים במשקל גוף על מוטות — נשאר טריצפס, וגם הוא לפי
      הכרטיס שלו: שם היחס הפוך (טרייספס 60%, חזה 25%). אלה שני תרגילים שונים,
      וההפרדה ביניהם כבר מתועדת ב-`UNLINKED_NOTES`.
    */
    muscleGroup: 'chest',
    subTarget: 'חזה תחתון וטריצפס',
    equipment: 'machine',
    weightMode: 'perSide',
    weightIncrementKg: 2.5,
    defaultRestSeconds: 120,
    ...
    targetReps: reps(8, 12),
    /*
      הדגש הראשון והשני התהפכו, ושניהם עדיין מה שהסרטון מלמד: הוא מסמן
      "the elbows out — pecs too much", כלומר מרפקים החוצה *הם* הגרסה של
      החזה. שני האחרונים נשארו — הם מכניקה של המכונה ולא של השריר.
    */
    cues: [
      'הטיה קדימה — זה מה שמעביר את העומס לחזה התחתון',
      'מרפקים מעט החוצה; זקוף עם מרפקים צמודים זו הגרסה של הטריצפס',
  ...
  /*
    ...
    תרגיל שנוסף לקטלוג בלי להשתייך ליום אימון. הוא בסוף המערך ולא בין תרגילי
    יד קדמית, וזה לא סגנון: `order` נגזר מהאינדקס כאן, והוספה באמצע הייתה
    מזיזה את מקומם של כל התרגילים שאחריו בהתקנה חדשה.

    בלי משקל זריעה. `hammer-curl` בישיבה הוא 17.5 אצל תבור, ובשיפוע היד מאחורי
    קו הגוף והתרגיל חלש ממנו במידה שאי אפשר לנחש — מספר שגוי כאן הופך להמלצה
    הראשונה שהמנוע ייתן. בלעדיו הוא אומר "אין עדיין היסטוריה, בחר משקל שנוח
    להתחיל בו", והסט הראשון הוא שקובע.
  */
  {
    id: 'incline-hammer-curl',
    name: 'כפיפת פטיש בשיפוע',
    nameEn: 'Incline Dumbbell Hammer Curl',
    muscleGroup: 'biceps',
    subTarget: 'ביצפס — ראש ארוך, וברכיורדיאליס',
    equipment: 'freeWeights',
    weightMode: 'total',
    weightIncrementKg: 2.5,
    defaultRestSeconds: 75,
    ...
    targetReps: reps(10, 15),
    /*
      ארבעת הדגשים מגיעים ממה שכתוב ומסומן על הסרטון עצמו: הזווית הנמדדת,
      החץ על הזרוע שנשארת מאחורי קו הגוף, והאזהרה על תנועת הזרוע העליונה.
      האחיזה מגיעה מהפריים.
    */
    cues: [
      'ספסל בשיפוע של 45–50 מעלות, הגב צמוד',
      'אחיזה ניטרלית — אגודלים למעלה לאורך כל התנועה',
      'הזרוע העליונה נשארת מאחורי קו הגוף, וזו כל המתיחה',
      'לא להניף את המרפק קדימה כדי לסיים את החזרה',
    ],
    usesPlates: false,
    barWeightKg: null,
    seedWeightKg: null,
  ...
  {
    id: 'ez-bar-curl',
    name: 'כפיפת מרפקים עם מוט דבליו',
    nameEn: 'EZ-Bar Curl',
    muscleGroup: 'biceps',
    // מהכרטיס: ראש קצר 40%, ראש ארוך 35%, ברכיאליס 15%, ברכיורדיאליס 10%
    subTarget: 'ביצפס',
    equipment: 'freeWeights',
    weightMode: 'total',
    weightIncrementKg: 2.5,
    ...
    targetReps: reps(8, 12),
    /*
      שני הראשונים הם מה שמסומן על הסרטון — חצים על המרפקים שנוסעים, כתפיים
      אדומות בעליית החזרה, והכיתוב "KEEP YOUR ELBOWS CLOSE TO YOUR BODY".
      האחרון הוא הסיבה להשתמש דווקא במוט הזה ולא במוט ישר.
    */
    cues: [
      'מרפקים צמודים לגוף — לא נוסעים קדימה ולא נפתחים לצדדים',
      'כתפיים למטה, בלי הרמה בסוף החזרה',
      'לרדת בשליטה עד מתיחה כמעט מלאה',
      'האחיזה הזוויתית של המוט נוחה יותר לשורש כף היד מאחיזה ישרה',
    ],
    /*
      ‏`usesPlates: false` בכוונה, אף שזה מוט טעון פלטות: משקל המוט עצמו משתנה
      בין חדר לחדר (‏7.5 עד 12 ק״ג), ומספר שגוי כאן נכנס לכל חישוב פלטות ולכל
      נפח. אפשר להדליק את המחשבון בעורך התרגיל ולמלא שם את המשקל הנכון.
    */
    usesPlates: false,
    barWeightKg: null,
    seedWeightKg: null,
...
/**
 * מה שהתקנה חדשה מקבלת: אותו קטלוג בדיוק — 31 תרגילים, אותם דגשים, סרטונים
 ...
 * שיתועד הוא שיקבע את המספר. זה מסלול קיים ומכוסה, לא חדש: שמונה מ-31
