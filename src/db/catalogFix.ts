...
/**
 * דור 16 — מקבילים במכונה חוזרים להיות תרגיל חזה.
 *
 * ‏`CATALOG_FIXES_V10` נתן לרשומה את השם והמיקוד שנגזרו מכותרת הסרטון. כרטיס
 * השרירים שנכנס למאגר מאז מודד חזה תחתון 48% מול טרייספס 27%, ולכן המיקוד
 * והשם האנגלי מתיישרים אחריו — וגם שני הדגשים הראשונים, שהתהפכו בסדר ולא
 * בתוכן. הסיווג עצמו אינו כאן אלא ב-`dipsChest.ts`: הוא רץ גם בייבוא גיבוי.
 *
 * ‏`was` הוא השם שמיגרציה 10 כתבה, ולכן מי שערך את השם בעצמו לא נגוע.
 */
export const CATALOG_FIXES_V16: readonly CatalogFix[] = [
  {
    id: 'dips',
    was: 'מקבילים במכונה',
    name: 'מקבילים במכונה',
