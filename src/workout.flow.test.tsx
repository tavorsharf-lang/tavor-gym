...
describe('זרימת אימון', () => {
  ...
  it('מתחילה אימון, מתעדת סט, ומסיימת עם אימון שמור בהיסטוריה', async () => {
    ...
    /*
      הכניסה דרך כרטיס תוכנית ולא דרך ההירו: "התחל אימון" הגדול מוביל עכשיו
      לבניית האימון — הדלת הראשית — וכרטיסי התוכניות הם הדרך לגיליון הבחירה.
      זה בדיוק המסלול שהבדיקה הזו מכסה: תוכנית קבועה, מהבית ועד סט שמור.
    */
    await user.click(screen.getAllByRole('button', { name: /פול באדי/ })[0])
