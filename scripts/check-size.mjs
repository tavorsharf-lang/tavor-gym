...
const LIMITS = [
  /*
    ‏2026-09-01: התקרה עלתה מ-1,050,000 ל-1,080,000.
    מיון וסינון ברשימות התרגילים הוסיפו כ-7KB לצ׳אנק הראשי — שכבת המיון,
    שורת הפקדים, ושלוש נקודות החיבור (מסך התרגילים, הבונה, והבורר). זה קוד
    אפליקציה ולא תלות חדשה, ולכן זה בדיוק הגידול שהתקרה אמורה *לאשר* אחרי
    שהיא עוצרת ושואלת. השער עשה את עבודתו: הוא חסם פריסה עד שמישהו הסביר.

    ‏2026-09-02: התקרה עלתה מ-1,080,000 ל-1,110,000.
    אימון חופשי — מסך בחירת השריר, רשת ההתאוששות, רשימת התרגילים, פאנל
    "מה הלאה" שבמנוחה, שבבי הכיול והתוכנית שנבנית מהסיכום — הוסיפו כ-12KB.
    שוב קוד אפליקציה ולא תלות: המנועים (`muscleCoverage`, `suggestWorkout`,
    `recommendWeight`) כבר היו בצ׳אנק, ומה שנוסף הוא המסכים שקוראים להם.
  */
  { label: 'הצ׳אנק הראשי', match: /^index-.*\.js$/, maxBytes: 1_110_000 },
